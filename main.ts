import { Plugin, Notice, MarkdownView, addIcon, Platform } from "obsidian";
...
export default class UpstackPlugin extends Plugin {
  ...
  async copySubstackHtml() {
    ...
    try {
      ...
      try {
        ...
        new Notice("Copied to clipboard for Substack");
      } catch (clipboardError) {
        // Fallback: Copy as plain text (for mobile compatibility)
        // Mobile browsers may not support ClipboardItem with HTML
        console.warn(
          "ClipboardItem not supported, falling back to plain text:",
          clipboardError,
        );
        try {
          const textContent = htmlOutput
            .replace(/<[^>]*>/g, "")
            .replace(/\s+/g, " ")
            .trim();
          await navigator.clipboard.writeText(textContent);
          new Notice(
            "Copied as plain text (HTML not supported on this platform)",
          );
        } catch (textError) {
          console.error("Failed to copy even as plain text:", textError);
          new Notice("Failed to copy. Check console for details.");
        }
      }
    } catch (err) {
      console.error("Upstack error:", err);
      new Notice("Failed to copy. Check console.");
    }
  }
  ...
  async processImages(
    ...
    console.debug("Found", matches.length, "image matches");
    ...
    for (let i = matches.length - 1; i >= 0; i--) {
      ...
      console.debug("Processing image match:", match[0]);
      ...
      try {
        ...
        if (!resolvedPath) {
          ...
          const imageData = await this.app.vault.adapter.readBinary(
            resolvedPath.path,
          );
          ...
          console.warn(
            "Resolved path exists but has no path property:",
            resolvedPath,
          );
        }
      } catch (error) {
  ...
  async transformForSubstack(md: string, view?: MarkdownView): Promise<string> {
    try {
      console.debug("Starting transformation...");

      // Step 1: Convert Mermaid charts to images using mermaid.ink API
      // Mermaid code blocks: ```mermaid ... ```
      const mermaidPattern = /```mermaid\n([\s\S]*?)```/g;
      md = md.replace(mermaidPattern, (_match, mermaidCode) => {
        console.debug("Found Mermaid chart, converting to image...");
        // Encode mermaid code to base64 for mermaid.ink API
        try {
          const bytes = new TextEncoder().encode(mermaidCode.trim());
          const binaryStr = Array.from(bytes, (b: number) =>
          ...
          console.debug("Mermaid PNG image URL generated");
          // Return as simple markdown image - let the image renderer handle it
          return `\n\n![Mermaid Chart](${mermaidImageUrl})\n\n`;
        } catch (error) {
          console.error("Error encoding Mermaid chart:", error);
      ...
      md = md.replace(excalidrawPattern, (_match, filePath, caption) => {
        console.debug("Found Excalidraw file:", filePath);
        // Extract just the filename without extension for a cleaner caption
        const fileName =
          filePath
            .split("/")
            .pop()
            ?.replace(/\.excalidraw$/, "") || "Excalidraw Drawing";
      ...
      md = md.replace(calloutPattern, (_match, type, content) => {
        console.debug("Found callout:", type);
...
${cleanContent}
      ...
      console.debug("All image references found:", allImageRefs);

      // Handle images with caption: ![[image.png|caption]]
      const imageWithCaptionPattern =
      ...
      const imageWithoutCaptionPattern =
        /!\[\[([^\]]+\.(png|jpg|jpeg|gif|svg|webp|bmp))\]\]/gi;
      const hasImagesWithoutCaption = imageWithoutCaptionPattern.test(md);
      imageWithoutCaptionPattern.lastIndex = 0; // Reset after test

      console.debug(
        "Checking for images - with caption:",
        hasImagesWithCaption,
        "without caption:",
        hasImagesWithoutCaption,
      );

      if (allImageRefs) {
        imageWithCaptionPattern.lastIndex = 0;
        const withCaptionMatches = Array.from(
          md.matchAll(imageWithCaptionPattern),
        );
        imageWithoutCaptionPattern.lastIndex = 0;
        const withoutCaptionMatches = Array.from(
          md.matchAll(imageWithoutCaptionPattern),
        );
        console.debug(
          "Pattern matches - with caption:",
          withCaptionMatches.map((m) => m[0]),
          "without caption:",
          withoutCaptionMatches.map((m) => m[0]),
        );
      }

      if (hasImagesWithCaption) {
        console.debug("Processing images with captions...");
        md = await this.processImages(md, imageWithCaptionPattern, view, true);
      }

      if (hasImagesWithoutCaption) {
        console.debug("Processing images without captions...");
        md = await this.processImages(
          md,
          imageWithoutCaptionPattern,
          view,
          false,
        );
      }
      ...
      // IMPORTANT: This must happen AFTER image processing
      ...
      renderer.strong = (token: Tokens.Strong) => {
        const text = originalStrong(token);
      ...
      renderer.blockquote = (token: Tokens.Blockquote) => {
        const text = originalBlockquote(token);
	...
	${caption ? `<figcaption style="text-align: center; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; font-size: 14px; color: #666; margin-top: 12px;">${caption}</figcaption>` : ""}
      ...
      renderer.link = (token: Tokens.Link) => {
        ...
        const originalHtml = originalLink(token);
      ...
      renderer.table = (token: Tokens.Table) => {
        console.debug(
          "Converting table to simple text format (Substack requires Datawrapper for tables)",
        );
        ...
        if (token.header && token.header.length > 0) {
          for (let i = 0; i < token.header.length; i++) {
            ...
            if (headerCell) {
              const cellHtml = originalTableCell({
                ...headerCell,
                header: true,
              });
        ...
        let output =
          '<p style="margin: 32px 0; font-family: Georgia, serif; font-size: 16px; color: #151515;">';
      ...
      renderer.tablerow = (token: Tokens.TableRow) => {
...
<span style="margin-left: 8px;">${content}</span>
      ...
      // Step 12: Extract title and build final output
      ...
      if (footnoteMap.size > 0) {
        let footnotesHtml =
          '<section style="margin-top: 48px; padding-top: 32px; border-top: 1px solid #e0e0e0;">';
        const sortedFootnotes = Array.from(footnoteMap.entries()).sort(
          (a, b) => parseInt(a[0]) - parseInt(b[0]),
        );

        for (const [id, content] of sortedFootnotes) {
          const footnoteContent = marked.parse(content, { async: false });
      ...
      // Output content directly without wrapper (Substack editor may strip wrapper divs)
      if (title) {
      ...
      // Final cleanup - ensure no extra whitespace
      html = html.trim();

      console.debug("Transformation complete");
      return html;
    } catch (error) {
      console.error("Error in transformForSubstack:", error);
      throw error;
    }
  }
}
